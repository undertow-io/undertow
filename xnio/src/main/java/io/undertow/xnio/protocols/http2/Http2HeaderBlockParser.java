    ...
    public void emitHeader(HttpString name, String value, boolean neverIndex) throws HpackException {
        if (maxHeaderListSize > 0) {
            ...
            if (headerSize > maxHeaderListSize) {
                throw new HpackException(UndertowXnioMessages.MESSAGES.headerBlockTooLarge(), Http2Channel.ERROR_PROTOCOL_ERROR);
            }
        }
        if (maxHeaders > 0 && headerMap.size() > maxHeaders) {
            return;
        ...
        if (name.length() == 0) {
            throw UndertowXnioMessages.MESSAGES.invalidHeader();
        }
        ...
        if (name.byteAt(0) == ':') {
            ...
            if (!processingPseudoHeaders) {
                throw new HpackException(UndertowXnioMessages.MESSAGES.pseudoHeaderInWrongOrder(name), Http2Channel.ERROR_PROTOCOL_ERROR);
            }
        } else {
        ...
        for (int i = 0; i < name.length(); ++i) {
            ...
            if (c >= 'A' && c <= 'Z') {
                invalid = true;
                UndertowXnioLogger.REQUEST_LOGGER.debugf("Malformed request, header %s contains uppercase characters", name);
            } else if (c != ':' && !HttpTokens.isValidTokenCharacter(c)) {
                invalid = true;
                UndertowXnioLogger.REQUEST_LOGGER.debugf("Malformed request, header %s contains invalid token character", name);
            }
        }
