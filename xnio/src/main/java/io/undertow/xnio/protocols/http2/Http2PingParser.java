...
import static io.undertow.xnio.protocols.http2.Http2Channel.PING_FRAME_LENGTH;
...
class Http2PingParser extends Http2PushBackParser {
    ...
    protected void handleData(ByteBuffer resource, Http2FrameHeaderParser parser) throws IOException {
        if (parser.length != PING_FRAME_LENGTH) {
            throw new IOException(UndertowXnioMessages.MESSAGES.httpPingDataMustBeLength8());
        }
        if (parser.streamId != 0) {
            throw new IOException(UndertowXnioMessages.MESSAGES.streamIdMustBeZeroForFrameType(Http2Channel.FRAME_TYPE_PING));
        }
