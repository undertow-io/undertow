...
public class Http2Channel extends AbstractFramedChannel<Http2Channel, AbstractHttp2StreamSourceChannel, AbstractHttp2StreamSinkChannel> implements Attachable {
    ...
    public Http2Channel(StreamConnection connectedStreamChannel, String protocol, ByteBufferPool bufferPool, PooledByteBuffer data, boolean clientSide, boolean fromUpgrade, UndertowOptionMap settings) {
        this(connectedStreamChannel, protocol, bufferPool, data, clientSide, fromUpgrade, true, null, settings);
    }

    public Http2Channel(StreamConnection connectedStreamChannel, String protocol, ByteBufferPool bufferPool, PooledByteBuffer data, boolean clientSide, boolean fromUpgrade, boolean prefaceRequired, UndertowOptionMap settings) {
        this(connectedStreamChannel, protocol, bufferPool, data, clientSide, fromUpgrade, prefaceRequired, null, settings);
    }

    public Http2Channel(StreamConnection connectedStreamChannel, String protocol, ByteBufferPool bufferPool, PooledByteBuffer data, boolean clientSide, boolean fromUpgrade, boolean prefaceRequired, ByteBuffer initialOtherSideSettings, UndertowOptionMap settings) {
    ...
    private void flushChannelIgnoreFailure(StreamSinkChannel stream) {
        try {
            ...
            UndertowXnioLogger.REQUEST_IO_LOGGER.ioException(e);
        } catch (Throwable t) {
            UndertowXnioLogger.REQUEST_IO_LOGGER.handleUnexpectedFailure(t);
        }
    }
    ...
    protected AbstractHttp2StreamSourceChannel createChannelImpl(FrameHeaderData frameHeaderData, PooledByteBuffer frameData) throws IOException {
        ...
        if (frameParser.type == FRAME_TYPE_DATA) {
            ...
            UndertowXnioLogger.REQUEST_LOGGER.tracef("Dropping Frame of length %s for stream %s", frameParser.getFrameLength(), frameParser.streamId);
            return null;
        }
        ...
        switch (frameParser.type) {
            ...
            case FRAME_TYPE_PUSH_PROMISE: {
                ...
                if (frameParser.parser instanceof Http2PushPromiseParser) {
                    if (!isClient()) {
                        sendGoAway(ERROR_PROTOCOL_ERROR);
                        throw UndertowXnioMessages.MESSAGES.serverReceivedPushPromise();
                    }
...
//                }
            ...
            case FRAME_TYPE_RST_STREAM: {
                ...
                if (frameParser.streamId == 0) {
                    ...
                    throw new ConnectionErrorException(Http2Channel.ERROR_PROTOCOL_ERROR, UndertowXnioMessages.MESSAGES.streamIdMustNotBeZeroForFrameType(FRAME_TYPE_RST_STREAM));
                }
...
//                //we don't return priority notifications, they are handled internally
            ...
            default: {
                UndertowXnioLogger.REQUEST_LOGGER.tracef("Dropping frame of length %s and type %s for stream %s as we do not understand this type of frame", frameParser.getFrameLength(), frameParser.type, frameParser.streamId);
                frameData.close();
                return null;
            }
        }
    ...
    protected FrameHeaderData parseFrame(ByteBuffer data) throws IOException {
        if (prefaceCount < PREFACE_BYTES.length) {
            while (data.hasRemaining() && prefaceCount < PREFACE_BYTES.length) {
                if (data.get() != PREFACE_BYTES[prefaceCount]) {
                    ...
                    throw UndertowXnioMessages.MESSAGES.incorrectHttp2Preface();
                }
                prefaceCount++;
            }
        }
        ...
        if (!initialSettingsReceived) {
            if (frameParser.type != FRAME_TYPE_SETTINGS) {
                UndertowXnioLogger.REQUEST_IO_LOGGER.remoteEndpointFailedToSendInitialSettings(frameParser.type);
        ...
        if (frameParser.getActualLength() > receiveMaxFrameSize && frameParser.getActualLength() > unackedReceiveMaxFrameSize) {
            sendGoAway(ERROR_FRAME_SIZE_ERROR);
            throw UndertowXnioMessages.MESSAGES.http2FrameTooLarge();
        }
    ...
    protected void handleBrokenSourceChannel(Throwable e) {
        UndertowXnioLogger.REQUEST_LOGGER.debugf(e, "Closing HTTP2 channel to %s due to broken read side", getPeerAddress());
    ...
    protected void handleBrokenSinkChannel(Throwable e) {
        UndertowXnioLogger.REQUEST_LOGGER.debugf(e, "Closing HTTP2 channel to %s due to broken write side", getPeerAddress());
        ...
        for (Http2Setting setting : settings) {
            if (setting.getId() == Http2Setting.SETTINGS_INITIAL_WINDOW_SIZE) {
                ...
                if (setting.getValue() > MAX_FRAME_SIZE || setting.getValue() < DEFAULT_MAX_FRAME_SIZE) {
                    UndertowXnioLogger.REQUEST_IO_LOGGER.debug("Invalid value received for SETTINGS_MAX_FRAME_SIZE " + setting.getValue());
                    sendGoAway(ERROR_PROTOCOL_ERROR);
                    return false;
                }
                ...
                if (result == 0) {
                    ...
                    //invalid value
                    UndertowXnioLogger.REQUEST_IO_LOGGER.debug("Invalid value received for SETTINGS_ENABLE_PUSH " + result);
                    sendGoAway(ERROR_PROTOCOL_ERROR);
                    return false;
                }
    ...
    public void handleWindowUpdate(int streamId, int deltaWindowSize) throws IOException {
        if (streamId == 0) {
            if (deltaWindowSize == 0) {
                UndertowXnioLogger.REQUEST_IO_LOGGER.debug("Invalid flow-control window increment of 0 received with WINDOW_UPDATE frame for connection");
                sendGoAway(ERROR_PROTOCOL_ERROR);
                return;
            }
            ...
            if (deltaWindowSize == 0) {
                UndertowXnioLogger.REQUEST_IO_LOGGER.debug("Invalid flow-control window increment of 0 received with WINDOW_UPDATE frame for stream " + streamId);
    ...
    void sendPing(byte[] data, final ChannelExceptionHandler<AbstractHttp2StreamSinkChannel> exceptionHandler, boolean ack) {
        ...
        try {
            ...
            if (exceptionHandler != null) {
                ...
                UndertowXnioLogger.REQUEST_LOGGER.debug("Failed to send ping and no exception handler set", e);
            }
        } catch (Throwable t) {
            if (exceptionHandler != null) {
                exceptionHandler.handleException(ping, new IOException(t));
            } else {
                UndertowXnioLogger.REQUEST_LOGGER.debug("Failed to send ping and no exception handler set", t);
            }
        }
    }
    ...
    public void sendGoAway(int status, final ChannelExceptionHandler<AbstractHttp2StreamSinkChannel> exceptionHandler) {
        ...
        if (UndertowXnioLogger.REQUEST_IO_LOGGER.isTraceEnabled()) {
            UndertowXnioLogger.REQUEST_IO_LOGGER.tracef(new ClosedChannelException(), "Sending goaway on channel %s", this);
        }
    ...
    public synchronized Http2HeadersStreamSinkChannel createStream(HeaderMap requestHeaders) throws IOException {
        if (!isClient()) {
            throw UndertowXnioMessages.MESSAGES.headersStreamCanOnlyBeCreatedByClient();
        }
        if (!isOpen()) {
        ...
        if (sendMaxConcurrentStreams > 0 && sendConcurrentStreams > sendMaxConcurrentStreams) {
            throw UndertowXnioMessages.MESSAGES.streamLimitExceeded();
        }
    ...
    public synchronized Http2HeadersStreamSinkChannel sendPushPromise(int associatedStreamId, HeaderMap requestHeaders, HeaderMap responseHeaders) throws IOException {
        ...
        if (isClient()) {
            throw UndertowXnioMessages.MESSAGES.pushPromiseCanOnlyBeCreatedByServer();
        }
        sendConcurrentStreamsAtomicUpdater.incrementAndGet(this);
        if (sendMaxConcurrentStreams > 0 && sendConcurrentStreams > sendMaxConcurrentStreams) {
            throw UndertowXnioMessages.MESSAGES.streamLimitExceeded();
    ...
    void registerStreamSink(Http2HeadersStreamSinkChannel synResponse) {
        ...
        if (existing == null) {
            throw UndertowXnioMessages.MESSAGES.streamNotRegistered();
        }
    ...
    public <T> T getAttachment(AttachmentKey<T> key) {
        if (key == null) {
            throw UndertowXnioMessages.MESSAGES.argumentCannotBeNull("key");
        }
    ...
    public void sendRstStream(int streamId, int statusCode) {
        ...
        if (UndertowXnioLogger.REQUEST_IO_LOGGER.isDebugEnabled()) {
            UndertowXnioLogger.REQUEST_IO_LOGGER.debugf(new ClosedChannelException(), "Sending rststream on channel %s stream %s", this, streamId);
        }
