...
import static io.undertow.xnio.protocols.http2.Hpack.HeaderField;
...
public class HpackDecoder {
    ...
    public void decode(ByteBuffer buffer, boolean moreData) throws HpackException {
        while (buffer.hasRemaining()) {
            ...
            if ((b & 0b10000000) != 0) {
                ...
                if (index == -1) {
                    if(!moreData) {
                        throw UndertowXnioMessages.MESSAGES.hpackFailed();
                    }
                    buffer.position(originalPos);
                    return;
                } else if(index == 0) {
                    throw UndertowXnioMessages.MESSAGES.zeroNotValidHeaderTableIndex();
                }
                handleIndex(index);
                ...
                throw UndertowXnioMessages.MESSAGES.invalidHpackEncoding(b);
            }
        }
        if(!moreData) {
            first = true;
    ...
    int getRealIndex(int index) throws HpackException {
        ...
        if(newIndex < 0) {
            throw UndertowXnioMessages.MESSAGES.invalidHpackIndex(index);
        }
        return newIndex;
    }
