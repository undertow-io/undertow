...
public class ChunkedStreamSourceConduit extends AbstractStreamSourceConduit<StreamSourceConduit> {
    ...
    private final ChunkedOperations chunkedOperations;
    ...
    public ChunkedStreamSourceConduit(final StreamSourceConduit next, final PushBackStreamSourceConduit channel, final ByteBufferPool pool, final ConduitListener<? super ChunkedStreamSourceConduit> finishListener, Attachable attachable, Closeable closeable) {
        this(next, new ChunkedOperations() {
            @Override
        ...
        }, finishListener, attachable);
    }

    public ChunkedStreamSourceConduit(final StreamSourceConduit next, final ChunkedOperations chunkedOperations, final ConduitListener<? super ChunkedStreamSourceConduit> finishListener, final Attachable attachable) {
        super(next);
        this.chunkedOperations = chunkedOperations;
    ...
    public long transferTo(final long position, final long count, final FileChannel target) throws IOException {
        try {
            ...
            IoUtils.safeClose(chunkedOperations);
            throw e;
        }
    }
    ...
    private void updateRemainingAllowed(final int written) throws IOException {
        if (remainingAllowed == Long.MIN_VALUE) {
            long maxEntitySize = chunkedOperations.getMaxEntitySize();
            if (maxEntitySize <= 0) {
                return;
            }
        ...
        if (remainingAllowed < 0) {
            ...
            chunkedOperations.requestToLarge();
            closed = true;
            finishListener.handleEvent(this);
            throw UndertowXnioMessages.MESSAGES.requestEntityWasTooLarge(chunkedOperations.getMaxEntitySize());
        }
    }
    ...
    public void terminateReads() throws IOException {
        if (!isFinished()) {
            ...
            throw UndertowXnioMessages.MESSAGES.chunkedChannelClosedMidChunk();
        }
    }
    ...
    public int read(final ByteBuffer dst) throws IOException {
        ...
        try {
            ...
            PooledByteBuffer pooled = chunkedOperations.allocate();
            ...
            try {
                ...
                if (!free && buf.hasRemaining()) {
                    chunkedOperations.pushBack(pooled);
                } else {
                    pooled.close();
                }
            }
    ...
    public interface ChunkedOperations extends Closeable {
