...
import io.undertow.xnio.client.http2.Http2ClearClientProvider;
import io.undertow.xnio.client.http2.Http2ClientConnection;
...
class HttpClientConnection extends AbstractAttachable implements Closeable, ClientConnection {
    ...
    private void handleError(IOException exception) {
        UndertowXnioLogger.REQUEST_IO_LOGGER.ioException(exception);
            ...
            try {
                ...
                if (builder == null) {
                    ...
                    try {
                        ...
                        if (res == -1) {
                            UndertowXnioLogger.CLIENT_LOGGER.debugf("Connection to %s was closed by the target server", connection.getPeerAddress());
                            safeClose(HttpClientConnection.this);
                        } else if (res != 0) {
                            UndertowXnioLogger.CLIENT_LOGGER.debugf("Target server %s sent unexpected data when no request pending, closing connection", connection.getPeerAddress());
                            safeClose(HttpClientConnection.this);
                        }
                        //otherwise it is a spurious notification
                    } catch (IOException e) {
                        if (UndertowXnioLogger.CLIENT_LOGGER.isDebugEnabled()) {
                            UndertowXnioLogger.CLIENT_LOGGER.debugf(e, "Connection closed with IOException");
                        }
                        safeClose(connection);
                    }
                    return;
                ...
                if (response.getResponseCode() == StatusCodes.SWITCHING_PROTOCOLS && Http2Channel.CLEARTEXT_UPGRADE_STRING.equals(response.getResponseHeaders().getFirst(Headers.UPGRADE))) {
                    ...
                    if (response.getResponseCode() == StatusCodes.EXPECTATION_FAILED) {
                        if (requiresContinueResponse(currentRequest.getRequest().getRequestHeaders())) {
                            HttpClientConnection.this.state |= CLOSE_REQ;
                ...
                UndertowXnioLogger.CLIENT_LOGGER.exceptionProcessingRequest(t);
                safeClose(connection);
                if (currentRequest != null) {
    ...
    private void prepareResponseChannel(ClientResponse response, ClientExchange exchange) {
        ...
        } else if (response.getProtocol().equals(Protocols.HTTP_1_1) && !isEntityBodyAllowed(response.getResponseCode())) {
            connection.getSourceChannel().setConduit(new FixedLengthStreamSourceConduit(connection.getSourceChannel().getConduit(), 0, responseFinishedListener));
        } else {
