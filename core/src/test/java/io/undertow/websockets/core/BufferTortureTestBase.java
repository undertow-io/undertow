...
public class BufferTortureTestBase {
    ...
    protected static final Set<String> allowedOrigins = new HashSet<>();
    ...
    static {
        allowedOrigins.add(VALID_ORIGIN);
    }
    ...
    public void test_1_baseline_Origin() throws Exception {
        ...
        try (WebSocketRawTestClient client = createRawClient()) {
            ...
            System.out.printf("  HTTP status: %d%n", result.statusCode());
            assertTrue("Valid Origin must be accepted — got " + result.statusCode(), result.isUpgraded());
    ...
    public void test_3_deflateFragment() throws Exception {
        ...
        try (WebSocketRawTestClient client = createRawClient()) {
            ...
            System.out.printf("  Close     : %d '%d' '%s' %n", x.opCode(), x.statusCode(), x.rawContent());
