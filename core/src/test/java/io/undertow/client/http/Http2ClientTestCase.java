...
public class Http2ClientTestCase {
    ...
    public static void setup() throws Exception {
        ...
        server = Undertow.builder()
                .addHttpListener(7773, "localhost")
                .addHttpsListener(7774, "localhost", sslContext)
    ...
    /**
     * It looks like there is no http/2 upgrade for http connection as we got some error. To force
     * http/2 connection for http request, please use h2c-prior schema.
     *
     * @throws Exception
     */
    @Test
    public void testSimpleHttpUpgrade() throws Exception {
        final UndertowClient client = createClient();

        final List<ClientResponse> responses = new CopyOnWriteArrayList<>();
        final CountDownLatch latch = new CountDownLatch(10);
        final ClientConnection connection = client.connect(new URI("h2c-prior://localhost:7773"), worker, DefaultServer.getBufferPool(), OptionMap.create(UndertowOptions.ENABLE_HTTP2, true)).get();
        try {
    ...
    public void testSimpleHttps() throws Exception {
        ...
        final ClientConnection connection = client.connect(new URI("https://localhost:7774"), worker, ssl, pool, OptionMap.create(UndertowOptions.ENABLE_HTTP2, true)).get();
        try {
            connection.getIoThread().execute(new Runnable() {
                @Override
                public void run() {
    ...
    public void testHttpGet() throws Exception {
        ...
        final ClientConnection connection = client.connect(new URI("http://localhost:7773"), worker, DefaultServer.getBufferPool(), OptionMap.create(UndertowOptions.ENABLE_HTTP2, true)).get();
        try {
    ...
    public void testHttpsPostRequest() throws Exception {
        ...
        final ClientConnection connection = client.connect(new URI("https://localhost:7774"), worker, ssl, DefaultServer.getBufferPool(), OptionMap.create(UndertowOptions.ENABLE_HTTP2, true)).get();
        try {
            connection.getIoThread().execute(new Runnable() {
                @Override
                public void run() {
