...
public class RequestContentEncodingTestCase3 {
    ...
    public static void setup() {
        ...
        final HttpHandler decode = new RequestEncodingHandler(exchange -> {
            exchange.startBlocking();
            Future<?> result = executor.submit(() -> exchange.getRequestReceiver()
                    .receiveFullBytes((exchange1, message) -> {
                        Assert.assertTrue(exchange1.getRequestContentLength() > 0);
                        exchange1.getResponseSender().send(ByteBuffer.wrap(message));
                    }));
            result.get();
