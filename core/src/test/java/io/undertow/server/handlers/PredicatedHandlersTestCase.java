...
public class PredicatedHandlersTestCase {
                ...
                Handlers.predicates(
                        PredicatedHandlersParser.parse(
                                "path(/skipallrules) and true -> done\n" +
                                        ...
                                        "regex('^/path/([^/]+)/(.*)/?$') -> rewrite('/newpath'); set(attribute='%{o,result}', value='param1=$1&param2=$2'); done()", getClass().getClassLoader()),
                        exchange -> exchange.getResponseSender().send(exchange.getRelativePath())));
    ...
    public void testRewriteContext() throws IOException {
        DefaultServer.setRootHandler(
                Handlers.predicates(
                        PredicatedHandlersParser.parse(
                                "path( foo ) -> rewrite( bar )", getClass().getClassLoader()),
                        exchange -> exchange.getResponseSender().send(exchange.getRelativePath())));
        try (CloseableHttpClient client = TestHttpClient.defaultClient()) {
    ...
    public void testSetHeader() throws Exception {
        ...
        SetAttributeHandler handler = (SetAttributeHandler) predicate.wrap(exchange -> {
        });
                ...
                Handlers.predicates(
                        PredicatedHandlersParser.parse(
                                "path(/skipallrules)\n and true -> done\n" +
                                        ...
                                        "regex('(.*).css') -> set[attribute='%{o,css}', value='true'] else set[attribute='%{o,css}', value='false']; " +
                                        "path(/restart) -> {rewrite(/foo/a/b); restart; }\r\n" +
                                        "regex('^/path/([^/]+)/(.*)/?$') \n-> rewrite('/newpath'); set(attribute='%{o,result}', value='param1=$1&param2=$2'); done()", getClass().getClassLoader()),
                        exchange -> exchange.getResponseSender().send(exchange.getRelativePath())));
    ...
    public void testReasonPhrase() throws IOException {
        DefaultServer.setRootHandler(
                Handlers.predicates(
                        PredicatedHandlersParser.parse(
                                "path('/test') -> reason-phrase('test-my-patience');response-code(480)", getClass().getClassLoader()),
                        exchange -> exchange.getResponseSender().send(exchange.getRelativePath())));
        try (CloseableHttpClient client = TestHttpClient.defaultClient()) {
    ...
    public void testDefaultResponse() throws IOException {
        DefaultServer.setRootHandler(
                Handlers.predicates(
                        PredicatedHandlersParser.parse(
                                "path('/test') -> response(code='208', reason='test-my-patience', body='Dont-Touch')", getClass().getClassLoader()),
                        exchange -> exchange.getResponseSender().send(exchange.getRelativePath())));
        try (CloseableHttpClient client = TestHttpClient.defaultClient()) {
            HttpGet get = new HttpGet(DefaultServer.getDefaultServerURL() + "/test");
    ...
    public void testCustomTypeResponse() throws IOException {
        DefaultServer.setRootHandler(
                Handlers.predicates(
                        PredicatedHandlersParser.parse(
                                "path('/test') -> response(code='208', reason='test-my-patience', body='Dont-Touch', type='text/plain')", getClass().getClassLoader()),
                        exchange -> exchange.getResponseSender().send(exchange.getRelativePath())));
        try (CloseableHttpClient client = TestHttpClient.defaultClient()) {
            HttpGet get = new HttpGet(DefaultServer.getDefaultServerURL() + "/test");
