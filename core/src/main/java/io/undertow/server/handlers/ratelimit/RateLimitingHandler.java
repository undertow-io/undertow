...
public class RateLimitingHandler implements HttpHandler {
    ...
    private RateLimiter<HttpServerExchange, HttpServerExchange> rateLimiter;
    ...
    public RateLimitingHandler(final HttpHandler nextHandler, final RateLimiter<HttpServerExchange, HttpServerExchange> rateLimiter) {
        super();
    ...
    public RateLimitingHandler(final HttpHandler nextHandler, final RateLimiter<HttpServerExchange, HttpServerExchange> rateLimiter, final String statusMessage,
    ...
    public void handleRequest(final HttpServerExchange exchange) throws Exception {
        ...
        final int currentRequestCount = rateLimiter.increment(exchange);
        ...
        if (this.signalLimits) {
            ...
            if (this.enforced) {
                responseHeaders.add(HttpString.tryFromString(HEADER_NAME_RATE_LIMIT), rateLimiter.getRemainingQuota(exchange));
            }
        }
        ...
        if (currentRequestCount > rateLimiter.getRequestLimit()) {
            UndertowLogger.REQUEST_LOGGER.exchangeExceedsRequestRateLimit(exchange.getRequestURI(), ipAddress,
                    rateLimiter.getRequestLimit(), rateLimiter.getWindowDuration(), rateLimiter.timeToWindowSlide(exchange), this.enforced);
            if (this.enforced) {
