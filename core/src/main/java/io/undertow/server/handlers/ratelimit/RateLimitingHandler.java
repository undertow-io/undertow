...
public class RateLimitingHandler implements HttpHandler {
    ...
    public RateLimitingHandler(final HttpHandler nextHandler, final RateLimiter rateLimiter) {
        super();
    ...
    public RateLimitingHandler(final HttpHandler nextHandler, final RateLimiter rateLimiter, final String statusMessage,
    ...
    public void handleRequest(final HttpServerExchange exchange) throws Exception {
        ...
        final int currentViolationCountr = rateLimiter.getCurrentViolationCount(key);
        if (currentViolationCountr > 0) {
            if(currentViolationCountr >= 2) {
                //if we get more violations, just debug.
                UndertowLogger.REQUEST_LOGGER.debugf("Request to '%s' from '%s' exceed rate limit of '%s' in time window of '%s'. Window will reset in '%s'. Rate limit is enforced '%s'.", new Object[] {exchange.getRequestURI(), ipAddress,
