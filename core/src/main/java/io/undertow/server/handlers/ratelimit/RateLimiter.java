...
public interface RateLimiter<K> {
    ...
    /**
     * Return estimated request limit. Estimated request limit(based on implementation strategy) can only reach
     * Integer.MAX_VALUE - 2. As increment/current are capped at Integer.MAX_VALUE.<br>
     * This allows to calculate number of offenses via {@link #current(Object)} - {@link #getRequestLimit()}. if this is not
     * honored, implementation must handle this accordingly, by overriding {@link #getCurrentViolationCount(Object)}.
     *
     * @return
     */
    int getRequestLimit();
    ...
    default String getRemainingQuota(K key) {
        ...
        final int t = timeToWindowSlide(key);
        return "\"" + getLimiterID() + "\";r=" + r + ";t=" + t;
    }

    /**
     * Return n+ for every request over limit
     * @return
     */
    default int getCurrentViolationCount(K key) {
        final int current = current(key); // -1 or positive.
        if (current == -1) {
            return 0;
        }
        final int violation = current(key) - getRequestLimit();
        if (violation > 0) {
            return violation;
        } else {
            return 0;
