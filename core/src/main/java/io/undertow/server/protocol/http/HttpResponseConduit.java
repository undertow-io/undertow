...
final class HttpResponseConduit extends AbstractStreamSinkConduit<StreamSinkConduit> {
    ...
    private static final AtomicIntegerFieldUpdater<HttpResponseConduit> stateUpdater = AtomicIntegerFieldUpdater.newUpdater(
            HttpResponseConduit.class, "state");
    ...
    void reset(HttpServerExchange exchange) {
        this.exchange = exchange;
        stateUpdater.set(this, STATE_START);
        fiCookie = -1L;
        string = null;
        headerValues = null;
        valueIdx = 0;
    ...
    private int processWrite(int state, final Object userData, int pos, int length) throws IOException {
        ...
        try {
                ...
                while (fiCookie != -1) {
                    ...
                    while (valueIdx < headerValues.size()) {
                        ...
                        if (remaining < 0) {
                            ...
                            this.charIndex = 0;
                            stateUpdater.set(this, STATE_HDR_NAME);
                            buffer.flip();
                        ...
                        if (remaining < 2) {//we use 2 here, to make sure we always have room for the final \r\n
                            ...
                            this.valueIdx = valueIdx;
                            this.charIndex = 0;
                            stateUpdater.set(this, STATE_HDR_VAL);
                            buffer.flip();
            ...
            if (buffer != null) {
                bufferDone();
                clearFlags(POOLED_BUFFER_IN_USE);
            }
        }
    }
